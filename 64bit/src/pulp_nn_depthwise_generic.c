...
void pulp_nn_depthwise_generic(
  ...
  uint8_t * bufferA = bufferC + (core_id * ((dim_kernel_x * (dim_im_in_y + padding_y_top + padding_y_bottom)) + dim_kernel_x));
  int i_out_x;
  int i_buff_y;
  ...
  for(int i_out_ch = start_channel; i_out_ch < stop_channel; i_out_ch++)
  {
    ...
    do
    {
      ...
      do
      {
        ...
        {
<<<<<<< HEAD
          *pOut = (uint8_t) clip8((sum + out_shift) >> out_shift);
=======
          *pOut = (uint8_t) clip8(sum >> out_shift);
>>>>>>> b9053e2a9e2c3d878d3ce564a9616dcd8c0feb2f
        }
        pOut+=(dim_im_out_x * ch_im_out);
        l++;
      }while(l<dim_im_out_y);
