...
%if config.out_data_t != 8 or config.wt_data_t != 8 or config.in_data_t != 8:
...
%endif
  ...
  int32_t *k1 = k + (core_id * ((ch_out >> Log2Core) + ((ch_out & (NUM_CORES - 1)) != 0)));
  int32_t *lambda1 = lambda + (core_id * ((ch_out >> Log2Core) + ((ch_out & (NUM_CORES - 1)) != 0)));
