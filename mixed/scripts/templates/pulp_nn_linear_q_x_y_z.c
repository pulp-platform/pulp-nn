...
%if config.out_data_t != 8 or config.wt_data_t != 8:
...
%endif
%if config.out_data_t == 4 and config.quantization == 'shift_clip':
#define clip8(x) __builtin_pulp_clipu_r(x, 15)
%elif config.out_data_t == 2 and config.quantization == 'shift_clip':
#define clip8(x) __builtin_pulp_clipu_r(x, 3)
%elif config.out_data_t == 8:
...
%elif config.out_data_t == 4:
	volatile int start = min((chunk << (chunk == 1)) * core_id, num_o_neurons);
	volatile int stop = min(start + (chunk << (chunk == 1)), num_o_neurons);
%elif config.out_data_t == 2:
	...
	volatile int start = min((chunk + neuron_left) * core_id, num_o_neurons);
	volatile int stop = min(start + chunk + neuron_left, num_o_neurons);
%endif
...
%elif config.out_data_t == 4:
	volatile uint8_t *pOut = (uint8_t *) pOutBuffer + (start >> 1);
%elif config.out_data_t == 2:
	volatile uint8_t *pOut = (uint8_t *) pOutBuffer + (start >> 2);
%endif
