...
from include.pulp_nn_struct import PULPNNSrcDirs, PULPNNSrcDirs32bit, PULPNNSrcDirs64bit, PULPNNInstallPath32bit, PULPNNInstallPath64bit, PULPNNInstallPath

##################################################################################### PULP-NN Factory ############################################################

class PULPNNKernel(object):
    def __init__(self, name, inp, out, wt, quant, act_prec):
        self.type = name
        self.in_data_t = inp
        self.out_data_t = out
        self.wt_data_t = wt
...
class PULPNNConvolvePointwise(PULPNNFactory):
    ...
    def generate_code(self):
        return Template(filename="templates/pulp_nn_conv_pointwise_x_y_z.t").render(config=self)
...
def headers(act_prec='32bit'):
    if act_prec == '32bit':
        shutil.copyfile(PULPNNSrcDirs32bit['inc'] + "pulp_nn_kernels.h", PULPNNSrcDirs32bit['pulp_nn_include'] + "pulp_nn_kernels.h")
        shutil.copyfile(PULPNNSrcDirs32bit['inc'] + "pulp_nn_utils.h", PULPNNSrcDirs32bit['pulp_nn_include'] + "pulp_nn_utils.h")
        shutil.copyfile(PULPNNSrcDirs32bit['support_function'] + "pulp_nn_utils.c", PULPNNSrcDirs32bit['pulp_nn_support_function'] + "pulp_nn_utils.c")
    elif act_prec == '64bit':
        shutil.copyfile(PULPNNSrcDirs64bit['inc'] + "pulp_nn_kernels.h", PULPNNSrcDirs64bit['pulp_nn_include'] + "pulp_nn_kernels.h")
        shutil.copyfile(PULPNNSrcDirs64bit['inc'] + "pulp_nn_utils.h", PULPNNSrcDirs64bit['pulp_nn_include'] + "pulp_nn_utils.h")
        shutil.copyfile(PULPNNSrcDirs64bit['support_function'] + "pulp_nn_utils.c", PULPNNSrcDirs64bit['pulp_nn_support_function'] + "pulp_nn_utils.c")
...
def utils(act_prec):
    comp = PULPNNUtils()
    if act_prec == '32bit':
        new_file_c = open(PULPNNSrcDirs32bit['support_function'] + comp.filename_c, 'w')
        new_file_h = open(PULPNNSrcDirs32bit['inc'] + comp.filename_h, 'w')
    elif act_prec == '64bit':
        new_file_c = open(PULPNNSrcDirs64bit['support_function'] + comp.filename_c, 'w')
        new_file_h = open(PULPNNSrcDirs64bit['inc'] + comp.filename_h, 'w')
...
def kernel(path_tag, comp, api):
    ...
    if comp.kernel.act_prec == '32bit':
        new_file = open(PULPNNSrcDirs32bit[path_tag] + comp.filename, 'w')
    elif comp.kernel.act_prec == '64bit':
        new_file = open(PULPNNSrcDirs64bit[path_tag] + comp.filename, 'w')
...
def matmul_mixed_tests_generator(layer, kernel):
    print("Matmul Mixed Test Generator (type: " + str(kernel.type) + ", bn: " + str(layer.bn) + ", relu: " + str(layer.relu) + ", quant: " + str(kernel.quantization) + ")")
    torch.manual_seed(5)
    random.seed(5)
    # input vectors
