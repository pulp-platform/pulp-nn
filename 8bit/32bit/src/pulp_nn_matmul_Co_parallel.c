...
uint8_t *pulp_nn_matmul_Co_parallel(
  ...
  int8_t  *pA = pWeight + num_col_im2col*start_channel;
  uint8_t *pB = pInBuffer;
  uint16_t chan_left = chunk & 0x3;
  ...
  /* this loop over the portion of channels */
  for (int i = start_channel; i < stop_channel>>2; i++)
  {
